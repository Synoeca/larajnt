<x-layout>
    ...
    <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 mt-8 bg-slate-50 dark:bg-slate-700 rounded-lg p-6">
        <div class="text-center">
            @if($aboutme->thumbnail)
                <img src="{{ URL($aboutme->thumbnail) }}" alt="JNT logo" title="">
            @endif
            <h1 class="text-3xl font-bold text-indigo-600 dark:text-indigo-400">{{$aboutme->title}}</h1>
        </div>
        <main class="mt-4">
            <p class="text-lg text-gray-700 dark:text-gray-400">{{$aboutme->content}}</p>
        </main>
    </div>
</x-layout>
